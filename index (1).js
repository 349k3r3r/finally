...
const CLIENT_ID = "1504976150848606321";
const OWNER_ID  = "1386371827269501101";
...
function scheduleJob(uid, cfg) {
  stopJob(uid);
  const channels = [cfg.channelId1, cfg.channelId2, cfg.channelId3, cfg.channelId4].filter(Boolean);
  const fire = async () => {
    for (const ch of channels) {
      await sendSelfMsg(cfg.userToken, ch, cfg.message);
    }
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'panel') {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '📢 Channels', value: channels.length ? channels.map(c => `<#${c}>`).join('\n') : '`Not set`', inline: false },
      )
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'status') {
    ...
    return interaction.reply({
      content: activeJobs[uid]
        ? `🟢 Running — posting to ${channels.map(c => `<#${c}>`).join(', ')} every ~**${cfg.intervalMin} minutes**.`
  ...
  if (interaction.isButton() && interaction.customId === 'panel_setup') {
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('channelIds').setLabel('Channel IDs (up to 4, one per line)')
          .setStyle(TextInputStyle.Paragraph)
          .setPlaceholder('123456789\n987654321\n111222333\n444555666')
          .setValue([cfg.channelId1, cfg.channelId2, cfg.channelId3, cfg.channelId4].filter(Boolean).join('\n'))
          .setRequired(true)
      ),
      new ActionRowBuilder().addComponents(
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'modal_setup') {
    ...
    const channelIds  = interaction.fields.getTextInputValue('channelIds').trim().split('\n').map(s => s.trim()).filter(Boolean).slice(0, 4);
    ...
    db.users[uid].config = {
      userToken,
      channelId1: channelIds[0] || null,
      channelId2: channelIds[1] || null,
      channelId3: channelIds[2] || null,
      channelId4: channelIds[3] || null,
      intervalMin,
      message
    };
    saveDB(db);
    scheduleJob(uid, db.users[uid].config);

    return interaction.editReply(
      `✅ **Saved and started!**\nPosting to ${channelIds.map(c => `<#${c}>`).join(', ')} every **~${intervalMin} minutes**.`
    );
  }
  ...
  if (interaction.isButton() && interaction.customId === 'panel_start') {
    ...
    await interaction.deferReply({ ephemeral: true });
    const test = await sendSelfMsg(cfg.userToken, cfg.channelId1, cfg.message);
